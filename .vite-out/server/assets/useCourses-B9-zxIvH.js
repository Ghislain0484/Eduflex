...
//#region src/hooks/useCourses.ts
var mapCourse = (row) => ({
	id: Number(row.id),
	userId: row.user_id,
	title: row.title,
...
function useCourses() {
	return useQuery({
		...
		queryFn: async () => {
			if (typeof window === "undefined") return DEFAULT_MOCK_COURSES;
			try {
          ...
          `).eq("status", "publie").order("created_at", { ascending: false });
...
function useCourse(id) {
	return useQuery({
		queryKey: ["courses", id],
		queryFn: async () => {
			if (typeof window === "undefined") return DEFAULT_MOCK_COURSES.find((c) => c.id === Number(id)) || null;
			try {
				const { data, error } = await supabase.from("courses").select(`
            *,
            profiles:user_id (
          ...
          `).eq("id", id).maybeSingle();
			...
			return DEFAULT_MOCK_COURSES.find((c) => c.id === Number(id)) || null;
		},
		enabled: id != null
	});
}
...
//#endregion
export { useUserCourses as i, useCourses as n, useManageCourses as r, useCourse as t };
